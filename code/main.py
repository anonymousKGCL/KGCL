...
for epoch in tqdm(range(world.TRAIN_epochs), disable=True):
    ...
    # kgc alternative training \ pretrain part
    if world.kgcontrast and not world.kgc_joint:
        cprint("[KG Contrast]")
        contrast_views = contrast_model.get_views("kg")

        cprint("[KG Readout and Iterate]")
        avg_loss = Procedure.train_contrast(Recmodel, contrast_model,
                                            contrast_views, optimizer)
        print(f"contrast loss: {avg_loss:.3f}")