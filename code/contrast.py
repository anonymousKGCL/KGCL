...
def drop_edge_random(edge_index, p):
    drop_mask = torch.empty(
        (edge_index.size(1), ), dtype=torch.float32,
        device=edge_index.device).uniform_(0, 1) < p
    x = edge_index.clone()
    x[:, drop_mask] = 0
    return x
...
def drop_edge_weighted(edge_index,
                       edge_weights,
                       p: float = 0.3,
                       threshold: float = 0.7):
...
class Contrast(nn.Module):
    ...
    def get_ui_views_pgrace(self):
        ...
        edge_weights = edge_weights.where(edge_weights < 1,
                                          torch.ones_like(edge_weights) * 1)
        # item_num
        drop_mask = torch.bernoulli(1. - edge_weights).to(
            torch.bool).cpu().tolist()
        ...
        tmp_adj = sp.csr_matrix(
            (ratings, (user_np, item_np + self.gcn_model.num_users)),
    ...
    def get_ui_views_weighted(self, item_stabilities, stab_weight):
        ...
        degree_weights = degree_weights.where(degree_weights > 0.3,
                                              torch.ones_like(degree_weights) *
                                              0.3)  # p_tau
        ...
        kg_weights = kg_weights.where(kg_weights > 0.3,
                                      torch.ones_like(kg_weights) * 0.3)

        # overall probability of keep
        weights = (1 - world.ui_p_drop) / torch.mean(
            stab_weight * kg_weights) * (stab_weight * kg_weights)
        # weights = weights.where(weights>0.3, torch.ones_like(weights) * 0.3)
        weights = weights.where(weights < 0.95,
                                torch.ones_like(weights) * 0.95)
    ...
    def ui_drop_random(self, p_drop):
        n_nodes = self.gcn_model.num_users + self.gcn_model.num_items
        keep_idx = randint_choice(
            len(self.gcn_model.dataset.trainUser),
            size=int(len(self.gcn_model.dataset.trainUser) * (1 - p_drop)),
            replace=False)
        user_np = self.gcn_model.dataset.trainUser[keep_idx]
    ...
    def get_views(self, aug_side="both"):
        ...
        if aug_side == "ui" or not world.kgcontrast:
            kgv1, kgv2 = None, None
        else:
