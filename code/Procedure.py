...
def Test_with_itemgroups(dataset, Recmodel, epoch, w=None, multicore=0):
    u_batch_size = world.config['test_u_batch_size']
    dataset: utils.BasicDataset
    ...
    if multicore == 1:
        pool = multiprocessing.Pool(CORES)
    for gi, group_items in enumerate(item_groups):
        with torch.no_grad():
            users = list(testDict.keys())
            try:
                assert u_batch_size <= len(users) / 10
            ...
            for batch_users in utils.minibatch(users, batch_size=u_batch_size):
                recall_batch = list()
                allPos = dataset.getUserPosItems(batch_users)
                groundTrue = [set(testDict[u])&group_items for u in batch_users]
                test_item_nums = np.array([len(u) for u in groundTrue])
                batch_users_gpu = torch.Tensor(batch_users).long()
                batch_users_gpu = batch_users_gpu.to(world.device)
                ...
                for range_i, items in enumerate(allPos):
                    ...
                    exclude_items.extend(items)
                rating[exclude_index, exclude_items] = -(1<<10)
                rating[:,list(all_items-group_items)] = -(1<<10)
                _, rating_K = torch.topk(rating, k=20)
                rating_K = rating_K.cpu().numpy()
                for i, j in enumerate(groundTrue):
                    recall_batch.append(len(set(list(rating_K[i])) & j))
                recall_batch = np.array(recall_batch)
                recall_list.append(np.mean(recall_batch/test_item_nums))
        results['recallg'+str(gi)] = np.mean(recall_list)
    if multicore == 1:
        pool.close()
    print(results)
    return results
